...
export default function Navbar() {
    ...
    const handleClickOutside = (event) => {
        ...
        else if (usericon.current && !usericon.current.contains(event.target)) {
            setShowuser(false); 
        }
    ...
    return (
        <nav id="navbar" className='mx-auto'>
            <div className='py-5 px-4 md:px-12 flex justify-between items-center'>
                <div className='flex gap-6 items-center '>
                    ...
                    <Link href="/promotion" className='cursor-pointer text-sm hover:underline underline-offset-4 decoration-4 md:block hidden'>โปรโมชั่น</Link>
                    <div className='search-box items-center border-white border-0.25 rounded-md bg-black gap-1 px-3 hidden f:flex md:ml-4'>
                        ...
                        <input type="text" className='search text-white opacity-100 p-1 ml-1 w-64 focus:outline-none bg-black' placeholder='ค้นหาหนัง' value={Search} onChange={(e) => setSearch(e.target.value)} onKeyDown={handlesearch}></input>
                    </div>
                </div>
                ...
                {isMobileNavVisible && (
                    <div className='fixed top-0 right-0 h-screen w-[325px] z-[999] bg-black md:hidden opacity-100 font-bold text-xl' ref={mobileNavRef}>
                        <div className='flex flex-col justify-start text-white p-6 gap-5'>
                            ...
                            {session ? (<div className='user-box'>
                                <h3 className='text-xl font-bold'>{session.user.username?session.user.username:"User"}</h3>

                                <div className='flex flex-col mt-3 gap-3 text-[14px]'>
                                    <Link href="/history" className='hover:underline underline-offset-4 decoration-4'>ประวัติการซื้อ</Link>
                                    <Link href="/promotion" className='hover:underline underline-offset-4 decoration-4'>โปรโมชั่น</Link>
                                    <Link href="/" onClick={() => signOut()} className='hover:underline underline-offset-4 decoration-4'>ออกจากระบบ</Link>
                                    {session.user.isAdmin && <Link href="/scanner">Scanner</Link>}
                                </div>
                            </div>) : (
                                <Link href="/login" onClick={(e) => setMobileNavVisible(false)} className='text-xl'>
                                    Sign In
                                </Link>
                            )}
                        </div>
                ...
                <div className='hidden md:flex text-md'>
                    {session ? (
                        <div ref={usericon} className='relative'>
                            ...
                            {showuser && (
                                <div className="absolute bg-white text-black mt-2 right-0 shadow-md rounded z-[999] p-2">
                                    ...
                                    <ul className='mt-3 cursor-pointer'>
                                        <Link href="/history"><li className="py-1 px-2 hover:bg-gray-200">ประวัติการซื้อ</li></Link>
                                        <Link href="/promotion"><li className="py-1 px-2 hover:bg-gray-200">โปรโมชั่น</li></Link>
                                        <li className="py-1 px-2  hover:bg-gray-200" onClick={() => signOut()}>ออกจากระบบ</li>
