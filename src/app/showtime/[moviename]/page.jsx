    ...
    export default function Showtime({ params }) {
        ...
        useEffect(() => {
            ...
            if (time) {
                setSelectedShowtime((prevState) => ({
                    ...prevState, // Keep all existing properties
                    Time: time, // Update or set only the `showtime` property
                }));
            }
            if (urlDate) {
                setDate(urlDate);
                                ...
                                {showtime.show_time.map((time) => {
                                    ...
                                    return (
                                        ...
                                        font-bold text-sm md:text-lg cursor-pointer ${isSelected ? "bg-gold" : "bg-white"} `}
                                            onClick={() => handleShowtimeClick(showtime.theater_name, time, index,showtime.Sub,showtime._id)}>
                                            {time}
                                        </div>
                                    )
                                })}
